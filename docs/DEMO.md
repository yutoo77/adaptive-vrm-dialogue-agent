...
3. Run `./start_demo.ps1` and confirm the compact `ローカル` provider badge and the selected VRM name, or the placeholder if no model is used.
...
Close with the test and evaluation evidence, then state the limitations: no Internet deployment, no streaming response, no semantic vector search, and no guarantee of transcription quality in noisy environments.

## One-minute route

1. State the problem and local-first policy in 10 seconds.
2. Send `何ができるの？` and show Text, VOICEVOX, expression, gesture, and lip sync in 30 seconds.
