...
hintは現在の配置への着眼点・具体的な見直し方・最終配置を段階的に開示する。手紙を調べる前のhintは調査を案内し、段階を進めない。
...
`experience_guidance.py`は、検証済みの3枠を読むだけの小さな関数。手紙の2条件を照合し、「丸い月が手元」「丸い月が左右」「真ん中は合っていて残りが未配置」「残りの前後が逆」「両条件に一致」を区別する。ゲーム状態を変更せず、最新の盤面について一つの次の着眼点を返す。

- ヒント1は観察と問いかけ、ヒント2は一段具体的な見直し。正しい部分を崩すようには案内しない。
- 誤答後も同じ照合を使い、配置とヒント段階を保持する。Mock相談・AI障害時も盤面に沿って返すが、自由文の意味理解を実装したわけではない。
- 正しい配置でも「条件には合いそう。箱を開けて確かめよう」と返すだけ。成功は従来どおりsubmitだけが確定する。
- ボタンは `ヒントを聞く → もう少しヒント → 答えを見る`。3段階目は短い確認dialogを挟み、初期Focusは「まだ考える」。取消/EscapeではRequest・進行・履歴を増やさない。開いた時のsession/revisionを照合し、古い確認操作を別の進行へ適用しない。
- 確認はUIの誤操作防止であり、APIの認証境界ではない。既存v1 APIの3回目のhintは最終配置を返す。開示済みの答えは同じボタンで再確認できる。
- 常設のボタン・説明カード・新しい依存は増やさない。無料の固定操作と既存API相談の区別を維持する。LLMの設定・呼出回数・送信範囲は変更しない。

選んだ理由は、3枚の固定問題には大きなAgent Frameworkも追加LLM判定も必要ないため。純粋関数（入力を読むだけで状態を変えない処理）に切り出すことで、全34通りの途中配置を検証できる。ヒントが適切な難しさか、相棒らしく感じるかは人の試遊で判断する。[検証記録](evaluations/board-aware-hints-2026-09-12.md)
