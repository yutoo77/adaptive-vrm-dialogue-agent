...
ヒントは今の盤面に合わせて、合っている部分と次に確かめる一点を短く返します。`ヒントを聞く → もう少しヒント → 答えを見る` の順に深まり、答えの開示前には確認できます。これらの操作はAPI課金なし。Mock相談やAPI障害時の固定応答も同じ盤面照合を使います。[盤面ヒントの検証](docs/evaluations/board-aware-hints-2026-09-12.md)
