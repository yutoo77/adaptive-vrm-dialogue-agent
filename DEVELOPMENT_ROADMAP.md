...
- 取得済みModelを同じBackendへ事前読込する任意の起動Option。通常の認識速度/品質設定は維持
...
- 実OpenAIは架空の一往復だけ追加確認。短い返答、うれしい/小さくうなずく、VOICEVOX再生中/終了後の復帰をEdgeで確認。音声入力は自作合成音声のLocal API確認であり、実マイク入力・長時間安定性・本人の主観的評価は未実施。
