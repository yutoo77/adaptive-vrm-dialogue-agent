...
- Backend: 176 tests、Ruff、pip check。Frontend: 160 tests、型/lint/build。Browser: 27 tests（通常対話を含む）。npm/pipの監査で既知脆弱性なし。
- 盤面照合、未調査時の非開示、誤答後の保持、API障害時の固定応答、答え確認の取消/Keyboard/320pxを確認。Edgeで実VRM表示とローカル音声の再生状態も確認。
- 実API本文生成・実マイク入力・長時間安定性・本人の主観的評価は今回未実施。起動直後の音声入力Health表示の不一致を別課題として記録する。
